...
#### Whereas a <code>TokenIndexer</code> represents a rule for how to turn a token into indices, a <code>Vocabulary</code> contains the corresponding mappings from strings to integers. For example, your token indexer might specify to represent a token as a sequence of character ids, in which case the <code>Vocabulary</code> would contain the mapping {character -> id}. In this particular example we use a <code>SingleIdTokenIndexer</code> that assigns each token a unique id, and so the <code>Vocabulary</code> will just contain a mapping {token -> id} (as well as the reverse mapping).
from allennlp.data.vocabulary import Vocabulary

#### Besides <code>DatasetReader</code>, the other class you'll typically need to implement is <code>Model</code>, which is a PyTorch <code>Module</code> that takes tensor inputs and produces a dict of tensor outputs (including the training <code>loss</code> you want to optimize).
