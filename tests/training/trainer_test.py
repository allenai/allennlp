...
@requires_multi_gpu
class TestDeepspeedTrainer(TrainerTestBase):
    @pytest.mark.parametrize(
        "batch_size, num_gradient_accumulation_steps", [(32, 1)]
    )
    def test_trainer_can_run_deepspeed(self, batch_size, num_gradient_accumulation_steps):
        import torch.multiprocessing as mp
        from allennlp.common import util as common_util

        node_rank = 0

        device_ids = [int(d) for d in os.getenv('CUDA_VISIBLE_DEVICES').split(',')]
...
def _test_worker(
    ...
    from allennlp.training.deepspeed.config import (
        DeepspeedConfig, 
        DeepspeedArgs,
        DeepspeedZeROConfig,
        DeepspeedFP16Config
    ...
    backend = "nccl" if process_rank >= 0 else "gloo"
    ...
    engine = DeepspeedTrainer._build_engine(
        model,
        optimizer,
        ...
        args=DeepspeedArgs(local_rank=process_rank),
