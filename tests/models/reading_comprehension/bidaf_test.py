...
class BidirectionalAttentionFlowTest(ModelTestCase):
    ...
    def test_get_best_span(self):
        ...
        span_begin_probs =torch.autograd.Variable(torch.FloatTensor([[0.4, 0.5, 0.1]])).log()
        span_end_probs =torch.autograd.Variable(torch.FloatTensor([[0.3, 0.6, 0.1]])).log()
        begin_end_idxs = BidirectionalAttentionFlow.get_best_span(span_begin_probs, span_end_probs)
        assert_almost_equal(begin_end_idxs.data.numpy(), [[1, 1]])

        # Another instance that used to be an edge case.
        span_begin_probs =torch.autograd.Variable(torch.FloatTensor([[0.8, 0.1, 0.1]])).log()
        span_end_probs =torch.autograd.Variable(torch.FloatTensor([[0.8, 0.1, 0.1]])).log()
        begin_end_idxs = BidirectionalAttentionFlow.get_best_span(span_begin_probs, span_end_probs)
        assert_almost_equal(begin_end_idxs.data.numpy(), [[0, 0]])

        span_begin_probs =torch.autograd.Variable(torch.FloatTensor([[0.1, 0.2, 0.05, 0.3, 0.25]])).log()
        span_end_probs =torch.autograd.Variable(torch.FloatTensor([[0.1, 0.2, 0.5, 0.05, 0.15]])).log()
        begin_end_idxs = BidirectionalAttentionFlow.get_best_span(span_begin_probs, span_end_probs)
