...
class PCABiasDirectionTest(AllenNlpTestCase):
    ...
    def test_pca_without_grad(self, device: str):
        seed_embeddings = torch.eye(2).to(device)
        pca = PCABiasDirection()

        const = 1 / math.sqrt(2)
        expected_bias_direction = torch.Tensor([const, -const]).to(device)
    ...
    def test_pca_with_grad(self, device: str):
        ...
        torch.manual_seed(0)
        seed_embeddings = torch.eye(2) + (1 - torch.eye(2)) * 1e-1
        seed_embeddings = seed_embeddings.requires_grad_().to(device)
        assert seed_embeddings.grad is None
...
class PairedPCABiasDirectionTest(AllenNlpTestCase):
    ...
    def test_paired_pca_without_grad(self, device: str):
        seed_embeddings1 = torch.eye(2).to(device)
        seed_embeddings2 = torch.tensor([[1.0, 1.0], [1.0, 1.0]]).to(device)
        paired_pca = PairedPCABiasDirection()
    ...
    def test_paired_pca_with_grad(self, device: str):
        # add noise to avoid "RuntimeError: triangular_solve_cpu: U(2,2) is zero, singular U."
        torch.manual_seed(0)
        seed_embeddings1 = torch.tensor([[1.0, 1.0], [1.0, 1.0]])
        seed_embeddings2 = (1 - torch.eye(2)) * 9e-1
        seed_embeddings1 = seed_embeddings1.requires_grad_().to(device)
        seed_embeddings2 = seed_embeddings2.requires_grad_().to(device)
        assert seed_embeddings1.grad is None
        assert seed_embeddings2.grad is None
...
class TwoMeansBiasDirectionTest(AllenNlpTestCase):
    ...
    def test_two_means_without_grad(self, device: str):
        seed_embeddings1 = torch.eye(2).to(device)
        seed_embeddings2 = (1 - torch.eye(2)).to(device)
        two_means = TwoMeansBiasDirection()

        expected_bias_direction = torch.Tensor([float("nan"), float("nan")]).to(device)
    ...
    def test_two_means_with_grad(self, device: str):
        seed_embeddings1 = torch.eye(2)
        seed_embeddings2 = 1 - torch.eye(2)
        seed_embeddings1 = seed_embeddings1.requires_grad_().to(device)
        seed_embeddings2 = seed_embeddings2.requires_grad_().to(device)
        assert seed_embeddings1.grad is None
        assert seed_embeddings2.grad is None
