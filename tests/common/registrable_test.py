...
class TestRegistrable(AllenNlpTestCase):
    ...
    def test_registry_has_builtin_samplers(self):
        assert PyTorchSampler.by_name("random").__name__ == "PyTorchRandomSampler"
        assert PyTorchSampler.by_name("sequential").__name__ == "PyTorchSequentialSampler"
        assert PyTorchBatchSampler.by_name("bucket").__name__ == "PyTorchBucketBatchSampler"
