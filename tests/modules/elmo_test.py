...
class TestElmoRequiresGrad(ElmoTestCase):
    def _run_test(self, requires_grad):
        ...
        batch_size = 3
        seq_len = 4
        char_ids =torch.autograd.Variable(torch.from_numpy(numpy.random.randint(0, 262, (batch_size, seq_len, 50))))
...
class TestElmoTokenRepresentation(ElmoTestCase):
    def test_elmo_token_representation(self):
        ...
        batch =torch.autograd.Variable(torch.from_numpy(numpy.array(sentences)))
    ...
    def test_elmo_token_representation_bos_eos(self):
        ...
        for correct_index, token in [[0, '<S>'], [2, '</S>']]:
            ...
            indices =torch.autograd.Variable(torch.from_numpy(numpy.array(indices))).view(1, 1, -1)
