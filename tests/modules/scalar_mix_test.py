...
class TestScalarMix(AllenNlpTestCase):
    def test_scalar_mix_can_run_forward(self):
        mixture = ScalarMix(3)
        tensors = [torch.autograd.Variable(torch.randn([3, 4, 5])) for _ in range(3)]
        for k in range(3):
    ...
    def test_scalar_mix_throws_error_on_incorrect_number_of_inputs(self):
        mixture = ScalarMix(3)
        tensors = [torch.autograd.Variable(torch.randn([3, 4, 5])) for _ in range(5)]
    ...
    def test_scalar_mix_layer_norm(self):
        ...
        numpy_mask[1, 2:] = 0
        mask =torch.autograd.Variable(torch.from_numpy(numpy_mask))
