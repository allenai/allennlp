...
class TestBidirectonalEndpointSpanExtractor:
    ...
    def test_correct_sequence_elements_are_embedded(self):
        ...
        correct_forward_start_indices =torch.autograd.Variable(torch.LongTensor([[0, 1],
                                                                   [-1, 2]]))
        ...
        correct_forward_end_indices =torch.autograd.Variable(torch.LongTensor([[3, 4], [2, 4]]))

        # Backward direction => start indices are exclusive, so add 1 to the end indices.
        correct_backward_start_indices =torch.autograd.Variable(torch.LongTensor([[4, 5], [3, 5]]))
        ...
        correct_backward_end_indices =torch.autograd.Variable(torch.LongTensor([[1, 2], [0, 3]]))
    ...
    def test_correct_sequence_elements_are_embedded_with_a_masked_sequence(self):
        ...
        # Forward direction => end indices are the same.
        correct_forward_end_indices =torch.autograd.Variable(torch.LongTensor([[3, 4], [2, 1]]))

        # Backward direction => start indices are exclusive, so add 1 to the end indices.
        correct_backward_start_indices =torch.autograd.Variable(torch.LongTensor([[4, 5], [3, 2]]))
        ...
        # Backward direction => end indices are inclusive and equal to the forward start indices.
        correct_backward_end_indices =torch.autograd.Variable(torch.LongTensor([[1, 2], [0, 0]]))
