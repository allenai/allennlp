...
class TestTimeDistributed(AllenNlpTestCase):
    def test_time_distributed_reshapes_correctly(self):
        ...
        char_input =torch.autograd.Variable(torch.LongTensor([[[1, 0], [1, 1]]]))
    ...
    def test_time_distributed_works_with_multiple_inputs(self):
        ...
        x_input =torch.autograd.Variable(torch.LongTensor([[[1, 2], [3, 4]]]))
        y_input =torch.autograd.Variable(torch.LongTensor([[[4, 2], [9, 1]]]))
