...
class TestDotProductSimilarityFunction(AllenNlpTestCase):
    def test_forward_does_a_dot_product(self):
        ...
        result = dot_product(torch.autograd.Variable(a_vectors),torch.autograd.Variable(b_vectors)).data.numpy()
    ...
    def test_forward_works_with_higher_order_tensors(self):
        ...
        result = dot_product(torch.autograd.Variable(torch.from_numpy(a_vectors)),
                            torch.autograd.Variable(torch.from_numpy(b_vectors))).data.numpy()
