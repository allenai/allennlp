...
class TestLinearSimilarityFunction(AllenNlpTestCase):
    ...
    def test_forward_does_a_weighted_product(self):
        ...
        result = linear(torch.autograd.Variable(a_vectors),torch.autograd.Variable(b_vectors)).data.numpy()
    ...
    def test_forward_works_with_higher_order_tensors(self):
        ...
        result = linear(torch.autograd.Variable(torch.from_numpy(a_vectors).float()),
                       torch.autograd.Variable(torch.from_numpy(b_vectors).float()))
    ...
    def test_forward_works_with_multiply_combinations(self):
        ...
        linear._bias = Parameter(torch.FloatTensor([0]))
        a_vectors =torch.autograd.Variable(torch.FloatTensor([[1, 1], [-1, -1]]))
        b_vectors =torch.autograd.Variable(torch.FloatTensor([[1, 0], [0, 1]]))
    ...
    def test_forward_works_with_divide_combinations(self):
        ...
        linear._bias = Parameter(torch.FloatTensor([0]))
        a_vectors =torch.autograd.Variable(torch.FloatTensor([[1, 1], [-1, -1]]))
        b_vectors =torch.autograd.Variable(torch.FloatTensor([[1, 2], [2, 1]]))
        result = linear(a_vectors, b_vectors).data.numpy()
        assert result.shape == (2,)
