...
from allennlp.nn.decoding import MaximumMarginalLikelihoodDecoder


class TestMaximumMarginalLikelihoodDecoder(AllenNlpTestCase):
    def test_create_allowed_transitions(self):
        ...
        targets = torch.Tensor([[1, 2, 3], [1, 4, 5], [1, 2, 7]])
        target_mask = torch.Tensor([[1, 1, 1], [1, 1, 1], [1, 1, 0]])
        result = MaximumMarginalLikelihoodDecoder._create_allowed_transitions(targets, target_mask)
        assert len(result) == 3
