                ...
                for alias_ind, alias in enumerate(answer['aliases']):
                    ...
                    if len(inds_not_found) > 0:
                        ...
                        updated_alias['token_answer_starts'] = [updated_alias['token_answer_starts'][ind] \
                                                                for ind in range(len(updated_alias['token_answer_starts'])) if
                                                                ind not in inds_not_found]
...
def main():
    ...
    parse.add_argument("--sort_by_question", type=str2bool, default=False,
                       help="sort by question token length to optimize GPU zero padding.")
