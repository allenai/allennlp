...
class ElmoEmbedder():
    ...
    def batch_to_embeddings(self, batch: List[List[str]]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # without_bos_eos is a 3 element list of (activation, mask) tensor pairs, each with size (batch_size, num_timesteps, dim)
        # and (batch_size, num_timesteps) respectively.
