    ...
    def test_unmasked_computation(self):
        ...
        for predictions, labels in predictions_labels_:
            ...
            for i in range(batch_size // stride):
                ...
                expected_spearman_correlation = spearman_corrcoef(predictions[:stride * (i + 1), :].reshape(-1),
                                                                  labels[:stride * (i + 1), :].reshape(-1))

                spearman_correlation(timestep_predictions, timestep_labels)
                assert_allclose(expected_spearman_correlation, spearman_correlation.get_metric(), rtol=1e-5)
    ...
    def test_masked_computation(self):
        ...
        for predictions, labels in predictions_labels_:
            ...
            for i in range(batch_size // stride):
                ...
                expected_spearman_correlation = spearman_corrcoef(predictions[:stride * (i+1), :].reshape(-1),
                                                                  labels[:stride * (i+1), :].reshape(-1),
                                                                  mask=mask[:stride * (i+1), :].reshape(-1))

                spearman_correlation(timestep_predictions, timestep_labels, timestep_mask)
