...
class SimpleSeq2SeqTest(ModelTestCase):
    ...
    def test_legacy_attention_model_can_train_save_and_load(self):
        param_overrides = json.dumps({"model": {"attention": None, "attention_function": {"type": "dot_product"}}})
        self.ensure_model_can_train_save_and_load(self.param_file, tolerance=1e-2, overrides=param_overrides)
    ...
    def test_loss_is_computed_correctly(self):
        ...
        sample_targets = torch.from_numpy(numpy.random.randint(0, num_classes, (batch_size, num_decoding_steps)))
    ...
    def test_greedy_decode_matches_beam_search(self):
        ...
        all_top_k_predictions, _ = beam_search.search(start_predictions, state, self.model.take_step)
