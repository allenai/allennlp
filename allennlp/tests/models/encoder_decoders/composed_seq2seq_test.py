...
class ComposedSeq2SeqTest(ModelTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.set_up_model(self.FIXTURES_ROOT / "encoder_decoder" / "composed_seq2seq" / "experiment.json",
    ...
    def test_loss_is_computed_correctly(self):
        ...
        actual_loss = self.model.decoder._get_loss(sample_logits, sample_targets, sample_mask)
    ...
    def test_greedy_decode_matches_beam_search(self):
        ...
        output_dict_greedy = self.model.decoder._forward_loss(state)
        output_dict_greedy = self.model.decoder.decode(output_dict_greedy)
