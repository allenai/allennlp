...
class Event2MindTest(ModelTestCase):
    ...
    def get_sample_encoded_output(self):
        ...
        source["tokens"]["tokens"] = source["tokens"]["tokens"].unsqueeze(0)
    ...
    def test_beam_search_orders_results(self):
        ...
        _, batch_logits = model._beam_search.search(start_predictions, start_state, state.take_step)
    ...
    def test_beam_search_matches_greedy(self):
        model = self.trained_model
        state = model._states["xintent"]
        beam_search = BeamSearch(model._end_index, max_steps=model._max_decoding_steps, beam_size=1)
        ...
        (beam_predictions, _) = beam_search.search(start_predictions, start_state, state.take_step)
