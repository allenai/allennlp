...
class TestAtisWorld(AllenNlpTestCase):
    ...
    def test_atis_global_actions(self):
        self.conv_context = ConversationContext()
        valid_actions = self.conv_context.valid_actions
    ...
    def test_atis_local_actions(self):
        ...
        self.conv_context = ConversationContext()
        world = AtisWorld(self.conv_context, "show me the flights from denver at 12 o'clock")
        ...
        self.conv_context.valid_actions = world.valid_actions

        world = AtisWorld(self.conv_context, "show me the delta or united flights in afternoon")
        ...
        self.conv_context.valid_actions = world.valid_actions
        world = AtisWorld(self.conv_context, "i would like one coach reservation for \
    ...
    def test_atis_simple_action_sequence(self):
        self.conv_context = ConversationContext()
        world = AtisWorld(self.conv_context, ("give me all flights from boston to "
        ...
        assert action_sequence == ['stmt -> [query, ";"]',
                                   'query -> ["(", "SELECT", distinct, select_results, ' +
                                   '"FROM", table_refs, where_clause, ")"]',
                                   'where_clause -> ["WHERE", conditions]',
                                   'conditions -> [condition]',
                                   'condition -> [in_clause]',
    ...
    def test_atis_long_action_sequence(self):
        self.conv_context = ConversationContext()
        world = AtisWorld(self.conv_context, ("what is the earliest flight in morning "
    ...
    def test_atis_from_json(self):
        ...
        self.conv_context = ConversationContext(line['interaction'])
        for interaction_round in self.conv_context.interaction:
            world = AtisWorld(self.conv_context, interaction_round['utterance'])
            world.get_action_sequence(interaction_round['sql'])
            self.conv_context.valid_actions = world.valid_actions
