...
class TestInitializers(AllenNlpTestCase):
    ...
    def test_prevent_regex_addition_works(self):
        # Same as above, except instead of having prevent regex in params,
        # add it in intializer applicator afterwards (using add_prevent_regexes).
        class Net(torch.nn.Module):
            def __init__(self):
                super(Net, self).__init__()
                self.linear_1 = torch.nn.Linear(5, 10)
        ...
        initializers = InitializerApplicator.from_params(params['initializer'])
        model = Net()
        initializers.add_prevent_regexes([".*_transfer.*", ".*pretrained.*"])
        initializers(model)
