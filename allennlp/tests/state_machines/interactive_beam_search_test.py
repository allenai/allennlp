...
class TestInteractiveBeamSearch(AllenNlpTestCase):
    def test_constraints(self):
        ...
        assert beam_search.choices == {
                1: [-1, -2],  # from -3, can go to -1 or -2
                2: [0, -1],   # forced to -2, can go to 0 or -1
                3: [1, 0],    # forced to -1, can go to 1 or 0
                4: [2, 1],    # forced to  0, can go to 2 or 1
                5: [3, 2],    # forced to  1, can go to 3 or 2
                6: [5, 4, 3],     # could go 2 -> {3, 4} or 3 -> {4, 5}
                7: [7, 6, 5, 4]   # could go 3 -> {4, 5} or 4 -> {5, 6} or 5 -> {6, 7}
        }
