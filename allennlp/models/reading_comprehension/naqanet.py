...
class NumericallyAugmentedQaNet(Model):
    ...
    def forward(
        ...
        if "counting" in self.answering_abilities:
            ...
            best_count_log_prob = torch.gather(count_number_log_probs, 1, best_count_number.unsqueeze(-1)).squeeze(
                -1
            )
            if len(self.answering_abilities) > 1:
        ...
        if "question_span_extraction" in self.answering_abilities:
            ...
            encoded_question_for_span_prediction = torch.cat(
                [encoded_question, passage_vector.unsqueeze(1).repeat(1, encoded_question.size(1), 1)], -1
            )
