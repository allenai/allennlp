...
class BidafPlusSelfAttentionBits(Model):
    ...
    def forward(self,  # type: ignore
        ...
        #mask3 = passage_mask.view(batch_size,-1,1).expand(batch_size,-1,3)
        #span_yesno_probs = util.masked_softmax(span_yesno_logits,mask3)
        #span_followup_probs = util.masked_softmax(span_followup_logits, mask3).squeeze(-1)
        ...
        output_dict = {"span_start_logits": span_start_logits,
                       ...
                       #"span_yesno_probs": span_yesno_probs,
                       "span_followup_logits": span_followup_logits,
                       #"span_followup_probs": span_followup_probs,
                       "best_span": best_span}
        if span_start is not None:
            ...
            loss += nll_loss(F.log_softmax(_yesno), yesno.squeeze(-1))
            loss += nll_loss(F.log_softmax(_followup), followup.squeeze(-1))
