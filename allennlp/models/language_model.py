...
class LanguageModel(Model):
    ...
    def forward(self,  # type: ignore
                inputs: Dict[str, torch.LongTensor] = None,
                forward_targets: Dict[str, torch.LongTensor] = None,
                backward_targets: Dict[str, torch.LongTensor] = None,
                source: Dict[str, torch.LongTensor] = None) -> Dict[str, torch.Tensor]:
        """
        ...
        inputs: ``torch.Tensor``, optional.
            The output of ``Batch.as_tensor_dict()`` for a batch of sentences, e.g.
            as generated by the LanguageModelingReader. This tensor represents the
            input tokens to be contextualized. If this is provided,
            ``source`` cannot be provided.
        forward_targets: ``torch.Tensor``, optional.
            The output of ``Batch.as_tensor_dict()`` for a batch of sentences, e.g.
            as generated by the LanguageModelingReader. This tensor represents the
            forward-direction targets corresponding to the ``inputs``. If this is
            provided, ``inputs`` must be provided (and ``source`` thus cannot be
            provided).
        backward_targets: ``torch.Tensor``, optional.
            The output of ``Batch.as_tensor_dict()`` for a batch of sentences, e.g.
            as generated by the LanguageModelingReader. This tensor represents the
            backward-direction targets corresponding to the ``inputs``. If this is
            provided, ``inputs`` must be provided (and ``source`` thus cannot be
            provided).
        source: ``torch.Tensor``, optional.
            The output of ``Batch.as_tensor_dict()`` for a batch of sentences, e.g.
            as generated by the SimpleLanguageModelingDatasetReader.
            If calculating the loss, we infer the prediction targets from this input,
            which is assumed to have a START and STOP symbol at the end of each instance.
            If this is provided, ``inputs`` or ``targets`` cannot be provided.
        ...
        if source is not None:
            # If source is provided, inputs, forward_targets, and backward_targets
            # should not be provided.
            if inputs is not None:
                raise ValueError("Received non-None values for both inputs and "
                                 "source. Only one should be provided.")
            if forward_targets is not None:
                raise ValueError("Received non-None values for 'source' and "
                                 "'forward_targets'. When using 'source', the labels "
                                 "are automatically inferred, so forward_targets "
                                 "should not provided. If you wish to use "
                                 "'forward_targets', provide the corresponding 'inputs' "
                                 "instead of using 'source'.")
            if backward_targets is not None:
                raise ValueError("Received non-None values for 'source' and "
                                 "'backward_targets'. When using 'source', the labels "
                                 "are automatically inferred, so backward_targets "
                                 "should not provided. If you wish to use "
                                 "'backward_targets', provide the corresponding 'inputs' "
                                 "instead of using 'source'.")

            # Calculate the forward_targets and backward_targets (if applicable) using
            # the source.
            token_ids = source.get("tokens")
            if token_ids is not None:
                # Get the targets if we have target tokens.
            ...
            # Set "source" to "inputs", for consistency below.
            inputs = source
        elif inputs is not None:
            # Get forward and backward targets from forward_targets
            # and backward_targets, if provided.
            if forward_targets is not None:
                forward_targets = forward_targets.get("tokens")
        ...
        # We only check forward_targets here, since they must always be provided
        # regardless of the bidirectionality of the language model.
        if forward_targets is not None:
