...
from allennlp.models.semantic_parsing.ratecalculus.ratecalculus_decoder_step import RateCalculusDecoderStep
...
@Model.register("ratecalculus_mml_parser")
class RateCalculusMmlSemanticParser(RateCalculusSemanticParser):
    """
    A ``RateCalculusMmlSemanticParser`` is a :class:`RateCalculusSemanticParser` which is trained to
    ...
    input_attention : ``Attention``
        ...
        decoder hidden state as the query.  Passed to RateCalculusDecoderStep.
    ...
    def forward(self,  # type: ignore
        ...
        world : ``List[RateCalculusWorld]``
            ...
            how ``MetadataField`` works, this gets passed to us as a ``List[RateCalculusWorld]``,
        actions : ``List[List[ProductionRuleArray]]``
        ...
        target_action_sequences : torch.Tensor, optional (default=None)
           ...
           sequence_length)``.
        """
        initial_info = self._get_initial_state_and_scores(question, question_knowledge_graph, world, actions)
            ...
            for i in range(batch_size):
                ...
                if i in best_final_states:
                    ...
                    for j in range(len(best_final_states[i])):
                        ...
                        action_strings.append([action_mapping[(i, action_index)] for action_index in best_action_indices])
                        try:
                            logical_form = world[i].get_logical_form(action_strings[0], add_var_function=False)
                            self._has_logical_form(1.0)
                        except ParsingError:
                            self._has_logical_form(0.0)
                            logical_form = 'Error producing logical form'
                        logical_forms.append(world[i].get_logical_form(action_strings[j], add_var_function=False))
                    ...
                    for lf in logical_forms:
                        print(lf)
                    outputs['logical_forms'].append(logical_forms)
                    outputs['debug_info'].append(best_final_states[i][0].debug_info[0])  # type: ignore
                    outputs['entities'].append(world[i].question_knowledge_graph.entities)
                else:
                    outputs['logical_forms'].append('')
                    self._has_logical_form(0.0)
                    if example_lisp_string:
    ...
    @classmethod
    def from_params(cls, vocab, params: Params) -> 'RateCalculusMmlSemanticParser':
