...
class WikiTablesDecoderStep(DecoderStep[WikiTablesDecoderState]):
    ...
    def take_step(self,
        ...
        previous_action_embedding = torch.stack([rnn_state.previous_action_embedding # Do we need to squeeze this here?
        ...
        for group_index in range(group_size):
            ...
            instance_action_ids = []
            output_action_embeddings = torch.zeros(0, dtype=torch.float)
            embedded_action_logits = torch.zeros(0, dtype=torch.float)
            ...
            if 'linked' in instance_actions:
                ...
                linked_action_logits = linking_scores.mm(attention_weights[group_index].unsqueeze(-1)).squeeze(-1) # Cast as a float to multiply with attention_weights
