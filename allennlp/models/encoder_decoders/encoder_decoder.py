...
class EncoderDecoder(Model):
    ...
    vocab : ``Vocabulary``, required
        ...
        or the target tokens can have a different namespace, in which case it needs to be specified as
        ``target_namespace``.
    ...
    target_namespace : str, optional (default = 'tokens')
        If the target side vocabulary is different from the source side's, you need to specify the target's
        namespace here. If not, we'll assume it is "tokens", which is also the default choice for the source side,
        and this might cause them to share vocabularies.
    target_embedding_dim : int, optional (default = source_embedding_dim)
        You can specify an embedding dimensionality for the target side. If not, we'll use the same value as the
        source embedder's.
    attention_function: ``SimilarityFunction``, optional (default = None)
        If you want to use attention to get a dynamic summary of the encoder outputs at each step of decoding, this
        is the function used to compute similarity between the decoder hidden state and encoder outputs.
    scheduled_sampling_ratio: float, optional (default = 0.0)
        ...
        this value, we use the ground truth labels for the whole batch. Else, we use the predictions from the
        previous time step for the whole batch. If this value is 0.0 (default), this corresponds to teacher
        forcing, and if it is 1.0, it corresponds to not using target side ground truth labels.
    ...
    def forward(self,  # type: ignore
        ...
        if target_tokens:
            ...
            # The last input from the target is either padding or the end symbol. Either way, we don't have to
            # process it.
    ...
    def _prepare_decode_step_input(self,
        ...
        if self._attention_function:
            ...
            # Ensuring mask is also a FloatTensor. Or else the multiplication within attention will complain.
    ...
    def from_params(cls, vocab, params: Params) -> 'EncoderDecoder':
        ...
        # If no attention function is specified, we should not use attention, not attention with default similarity
        # function.
