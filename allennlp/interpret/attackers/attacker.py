...
class Attacker(Registrable):
    ...
    def attack_from_instance(
        ...
        """
        This function finds a modification to the input instance that would change the model's
        prediction in some desired manner (e.g., an adversarial attack).

        Parameters
        ----------
        instance : ``Instance``
            The instance you want to attack (the same as the argument to a Predictor, e.g.,
            predict_instance()).
        input_field_to_attack : ``str``
            The key in the inputs JsonDict you want to attack, e.g., ``tokens``.
        grad_input_field : ``str``
