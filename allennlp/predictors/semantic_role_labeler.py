...
class SemanticRoleLabelerPredictor(Predictor):
    ...
    def predict_batch_json(self, inputs: List[JsonDict]) -> List[JsonDict]:
        ...
        flattened_instances = [
            instance for sentence_instances in instances_per_sentence for instance in sentence_instances
        ]

        if not flattened_instances:
            return sanitize([{"verbs": [], "words": self._tokenizer.split_words(x["sentence"])} for x in inputs])
    ...
    def predict_instances(self, instances: List[Instance]) -> JsonDict:
        ...
        for output in outputs:
            ...
            results["verbs"].append({"verb": output["verb"], "description": description, "tags": tags})
