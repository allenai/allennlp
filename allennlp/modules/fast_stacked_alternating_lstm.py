...
from allennlp.modules import RecurrentHighway, TimeDistributed
from allennlp.common.checks import ConfigurationError


class FastStackedAlternatingLstm(torch.nn.Module):
    """
    ...
    def __init__(self,
        ...
        super(FastStackedAlternatingLstm, self).__init__()
        ...
        layers = []
        highway_layers = [] if use_highway else None
        lstm_input_size = input_size
        for layer_index in range(num_layers):
            layer = LSTM(lstm_input_size, hidden_size)
            highway_layer = TimeDistributed(RecurrentHighway(lstm_input_size, hidden_size))
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        final_states = []
        for layer, highway, state in zip(self.lstm_layers, self.highway_layers, hidden_states):
            # The state is duplicated to mirror the Pytorch API for LSTMs.
            input_cache = output_sequence
            output_sequence, final_state = layer(output_sequence, state)
            output_sequence = highway(input_cache, output_sequence)
