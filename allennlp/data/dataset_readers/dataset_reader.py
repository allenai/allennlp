import itertools
from typing import Iterable, Iterator, Optional, List, Any, Callable, Union, Dict, Tuple
import logging
import os
import pathlib
...
T = Union[Instance, Dict[str, Any], Tuple[Any, ...]]
...
class DatasetReader(Registrable):
    ...
    def _read(self, file_path: str) -> Iterable[T]:
        """
        Generates objects that can be turned into `Instance`s from the given `file_path`.

        Each object generated by this method is transformed into an instance
        by the `DatasetReader.ensure_instance` method. The default implementation of
        `ensure_instance` can handle several different data types, each of which are
        interpreted differently.

        Namely, `tuple`s are interpreted as positional arguments for `DatasetReader.text_to_instance`,
        ...
        delegating any expensive work to `DatasetReader.text_to_instance`.
        """
    ...
    def ensure_instance(self, raw_data: T) -> Instance:
        """
        ...
        if isinstance(raw_data, Instance):
            ...
            return self.text_to_instance(**raw_data)
        raise TypeError(
            f"Unexpected type: {type(raw_data)}. You may need to override 'DatasetReader.ensure_instance'"
        )
    ...
    def multi_worker_islice(
        ...
        transform : `Optional[Callable[[Any], Instance]]`, optional (default = `None`)
            ...
            `ensure_instance` method is used.
