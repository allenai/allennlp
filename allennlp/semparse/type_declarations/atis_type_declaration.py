...
from allennlp.semparse.type_declarations.type_declaration import ComplexType, HigherOrderType, NamedBasicType, HierarchalNamedBasicType

# Basic Types
NUM_TYPE = NamedBasicType("NUM")
STRING_TYPE = NamedBasicType("STRING")
ENTITY_TYPE = HierarchalNamedBasicType("ENTITY", ["NUM", "STRING", "BOOL"])
...
BINOP_TYPE = ComplexType(ENTITY_TYPE, ComplexType(ENTITY_TYPE, TRUTH_TYPE))

SELECT_TYPE = ComplexType(STRING_TYPE, ComplexType(FROM_BASIC_TYPE, ComplexType(WHERE_BASIC_TYPE, ENTITY_TYPE))) 
SELECT_DISTINCT_TYPE = ComplexType(STRING_TYPE, ComplexType(FROM_BASIC_TYPE, ComplexType(WHERE_BASIC_TYPE, ENTITY_TYPE))) 

FROM_TYPE = ComplexType(STRING_TYPE, FROM_BASIC_TYPE)
WHERE_TYPE = ComplexType(TRUTH_TYPE, WHERE_BASIC_TYPE)

IN_TYPE = ComplexType(STRING_TYPE, ComplexType(ENTITY_TYPE, TRUTH_TYPE))
...
add_common_name_with_type("SELECT_DISTINCT", "S1", SELECT_DISTINCT_TYPE)
...
add_common_name_with_type("=", "B1", BINOP_TYPE)


add_common_name_with_type("IN", "I0", IN_TYPE)
