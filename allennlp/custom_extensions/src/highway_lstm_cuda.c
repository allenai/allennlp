...
int highway_lstm_forward_cuda(int inputSize, int hiddenSize, int miniBatch,
                              int numLayers, int seqLength, THCudaTensor *x,
                              THIntTensor *lengths, THCudaTensor *h_data,
                              THCudaTensor *c_data, THCudaTensor *tmp_i,
                              THCudaTensor *tmp_h, THCudaTensor *T,
                              THCudaTensor *bias, THCudaTensor *dropout,
                              float dropout_p, THCudaTensor *gates,
                              int isTraining) {
  ...
  highway_lstm_forward_ongpu(
      inputSize, hiddenSize, miniBatch, numLayers, seqLength, x_ptr,
      lengths_ptr, h_data_ptr, c_data_ptr, tmp_i_ptr, tmp_h_ptr, T_ptr,
      bias_ptr, dropout_ptr, dropout_p, gates_ptr, isTraining, stream, handle);

  return 1;
}

int highway_lstm_backward_cuda(
    int inputSize, int hiddenSize, int miniBatch, int numLayers, int seqLength,
    THCudaTensor *out_grad, THIntTensor *lengths, THCudaTensor *h_data_grad,
    THCudaTensor *c_data_grad, THCudaTensor *x, THCudaTensor *h_data,
    THCudaTensor *c_data, THCudaTensor *T, THCudaTensor *gates_out,
    THCudaTensor *dropout_in, float dropout_p, THCudaTensor *h_gates_grad,
    THCudaTensor *i_gates_grad, THCudaTensor *ones, THCudaTensor *h_out_grad,
    THCudaTensor *x_grad, THCudaTensor *T_grad, THCudaTensor *bias_grad,
  ...
  highway_lstm_backward_ongpu(
      inputSize, hiddenSize, miniBatch, numLayers, seqLength, out_grad_ptr,
      lengths_ptr, h_data_grad_ptr, c_data_grad_ptr, x_ptr, h_data_ptr,
      c_data_ptr, T_ptr, gates_out_ptr, dropout_in_ptr, dropout_p,
      h_gates_grad_ptr, i_gates_grad_ptr, ones_ptr, h_out_grad_ptr, x_grad_ptr,
      T_grad_ptr, bias_grad_ptr, isTraining, do_weight_grad, stream, handle);
