...
__global__ void elementWise_fp(int hiddenSize, int miniBatch, int numCovered,
  ...
  for (int i = 0; i < 5; i++) {
    g[i] = tmp_i[i * hiddenSize + i_gateIndex] +
           tmp_h[i * hiddenSize + h_gateIndex];
...
void highway_lstm_backward_ongpu(
    ...
    float * __restrict__ h_data_grad,
    float * __restrict__ c_data_grad,
    float * __restrict__ x,
    float * __restrict__ h_data,
    float * __restrict__ c_data,
    float * __restrict__ T,
    float * __restrict__ gates_out,
    float * __restrict__ dropout_in,
    float dropout_p,
    float * __restrict__ h_gates_grad,
    float * __restrict__ i_gates_grad,
    float * __restrict__ ones,
    float * __restrict__ h_out_grad,
    float * __restrict__ x_grad,
    float * __restrict__ T_grad,
  ...
  for (int layer = numLayers - 1; layer >= 0; layer--) {
    ...
    for (int t = startInd; t < seqLength && t >= 0; t = t + direction) {
      ...
      if (layer == numLayers - 1) {
        ...
        gradPtr =
            h_out_grad + t * numElements + layer * seqLength * numElements;
      }
      ...
      elementWise_bp<<<gridDim, blockDim, 0, stream>>>(
          hiddenSize, miniBatch, currNumCovered, gradPtr,
          h_data_grad + prevGradIndex * numElements +
              layer * (seqLength + 1) * numElements,
          c_data_grad + prevGradIndex * numElements +
              layer * (seqLength + 1) * numElements,
          c_data + prevIndex * numElements +
              layer * (seqLength + 1) * numElements,
          c_data + (t + 1) * numElements +
              layer * (seqLength + 1) * numElements,
          h_data + (t + 1) * numElements +
              layer * (seqLength + 1) * numElements,
          gates_out + t * 6 * numElements + layer * seqLength * 6 * numElements,
          dropout_in + layer * numElements, dropout_p,
          c_data_grad + (t + 1) * numElements +
              layer * (seqLength + 1) * numElements,
          i_gates_grad, h_gates_grad, isTraining);
      ...
      if (layer == 0) {
        ...
        inSize = hiddenSize;
        out_grad_ptr = h_out_grad + t * numElements +
                       (layer - 1) * seqLength * numElements;
        weightStart = 6 * hiddenSize * inputSize + 5 * hiddenSize * hiddenSize +
                      (layer - 1) * 11 * hiddenSize * hiddenSize;
      }

      cublasErrCheck(cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, inSize,
                                 currNumCovered, 6 * hiddenSize, &one,
                                 &T[weightStart], 6 * hiddenSize, i_gates_grad,
                                 6 * hiddenSize, &zero, out_grad_ptr, inSize));

      cublasErrCheck(cublasSgemm(
          handle, CUBLAS_OP_T, CUBLAS_OP_N, hiddenSize, currNumCovered,
          5 * hiddenSize, &one, &T[weightStart + 6 * hiddenSize * inSize],
          5 * hiddenSize, h_gates_grad, 5 * hiddenSize, &zero,
          h_data_grad + (t + 1) * numElements +
              layer * (seqLength + 1) * numElements,
          hiddenSize));
      ...
      if (do_weight_grad == 1) {
        ...
        if (layer == 0) {
          ...
          inputPtr = h_data + (t + 1) * numElements +
                     (layer - 1) * (seqLength + 1) * numElements;
        }

        // Update i_weights
        cublasErrCheck(cublasSgemm(
            handle, CUBLAS_OP_N, CUBLAS_OP_T, 6 * hiddenSize, inSize,
            currNumCovered, &one, i_gates_grad, 6 * hiddenSize, inputPtr,
            inSize, &one, &T_grad[weightStart], 6 * hiddenSize));

        // Update h_weights
        cublasErrCheck(cublasSgemm(
            handle, CUBLAS_OP_N, CUBLAS_OP_T, 5 * hiddenSize, hiddenSize,
            currNumCovered, &one, h_gates_grad, 5 * hiddenSize,
            h_data + prevIndex * numElements +
                layer * (seqLength + 1) * numElements,
            hiddenSize, &one, &T_grad[weightStart + 6 * hiddenSize * inSize],
            5 * hiddenSize));

        // Update bias_weights
        cublasErrCheck(cublasSgemv(handle, CUBLAS_OP_N, 5 * hiddenSize,
                                   currNumCovered, &one, h_gates_grad,
                                   5 * hiddenSize, ones, 1, &one,
                                   &bias_grad[layer * 5 * hiddenSize], 1));
      }
    }
  }
...
void highway_lstm_forward_ongpu(int inputSize, int hiddenSize, int miniBatch,
                                ...
                                float * __restrict__ dropout, float dropout_p,
                                float * __restrict__ gates, int is_training,
                                cudaStream_t stream, cublasHandle_t handle) {
  ...
  for (int layer = 0; layer < numLayers; layer++) {
    ...
    for (int t = startInd; t < seqLength && t >= 0; t = t + direction) {
      ...
      cublasErrCheck(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
                                 6 * hiddenSize, currNumCovered, inSize, &one,
                                 &T[weightStart], 6 * hiddenSize, inputPtr,
                                 inSize, &zero, tmp_i, 6 * hiddenSize));

      cublasErrCheck(cublasSgemm(
          handle, CUBLAS_OP_N, CUBLAS_OP_N, 5 * hiddenSize, currNumCovered,
          hiddenSize, &one, &T[6 * hiddenSize * inSize + weightStart],
          5 * hiddenSize, h_data + prevIndex * numElements +
                              layer * (seqLength + 1) * numElements,
          hiddenSize, &zero, tmp_h, 5 * hiddenSize));
      ...
      elementWise_fp<<<gridDim, blockDim, 0, stream>>>(
          hiddenSize, miniBatch, currNumCovered, tmp_h, tmp_i,
          ...
          h_data + (t + 1) * numElements +
              layer * (seqLength + 1) * numElements,
          dropout + layer * numElements, dropout_p,
          c_data + prevIndex * numElements +
              layer * (seqLength + 1) * numElements,
          c_data + (t + 1) * numElements +
              layer * (seqLength + 1) * numElements,
